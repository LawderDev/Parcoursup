<template>
    <div>
      <Modal @close="$emit('update:isOpen', false)">
        <template v-slot:open-btn>
          <div v-if="hideButton" class="hidden" ref="openBtn"></div>
          ...
          <div class="flex flex-col">
            ...
            <div class="text-secondary">
              <p class="mb-2 text-center">
                Voulez-vous supprimer l'utilisateur {{ name }} {{ firstname }} ?
              </p>
            </div>
          </div>
        </template>
  ...
  const handleSubmit = async () => {
    try {
      await axios.post(`${config.public.backUrl}/api/delete_user`, {
        userID: props.userId,
      });
      emit("handleDelete");
      toaster.showMessage("L'utilisateur a bien été supprimée", "success");
  
    } catch (error) {
      emit("handleDelete");
      console.error("Erreur lors de la suppression de l'utilisateur", "error");
      toaster.showMessage("Erreur lors de la suppression de l'utilisateur", "error");
    }
  };
  </script>
